...
def is_correct(btn):
    ...
    if btn.text() == parameters["correct"][-1]:
        ...
        #update score (+2.5 points)
        temp_score = parameters["score"][-1]
        parameters["score"].pop()
        parameters["score"].append(temp_score + 2.5)
...
def frame4():
    #sorry widget
    message = QLabel(f'Sorry, this answer was wrong.\nThe correct answer is {parameters["correct"][-1]}')
    message.setAlignment(QtCore.Qt.AlignRight)
    message.setStyleSheet(
        "font-family: 'Shanti'; font-size: 30px; color: 'white'; margin: 75px 5px; padding:20px;"
        )
    widgets["message"].append(message)
